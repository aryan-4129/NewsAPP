...
const News = ({ category }) => {
  ...
  const fetchURL = async () => {
    ...
    }&country=in&language=en,hi${
      nextPageObj.index > -1
        ? "&page=" + nextPageObj.arr[nextPageObj.index]
        : ""
    }`;
    setloader(true);
  ...
  const removeLastOfArr = (arr, index) => {
    const newArr = arr.filter(
      (item) => item != arr[index - 1] && item != arr[index]
    );
    return newArr;
  };
  ...
  return (
    <>
      <center>
        {loader && (
          <button
            className={`btn btn-primary ${css.loaderButton}`}
            type="button"
            disabled
          >
            <span
        ...
        {!loader && (
          <div className="buttonContainer">
            <FaArrowLeft className={css.button} onClick={handlePrevButton} />
            <FaArrowRight className={css.button} onClick={handleNextButton} />
          </div>
        )}
      </center>
    </>
